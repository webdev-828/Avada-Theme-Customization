/*global avadaredux_change, wp, avadaredux, libFilter */
...
(function($){
    ...
    avadaredux.field_objects.media   = avadaredux.field_objects.media || {};
    ...
    avadaredux.field_objects.media.init = function( selector ) {
        if ( !selector ) {
            selector = $( document ).find( ".avadaredux-group-tab:visible" ).find( '.avadaredux-container-media:visible' );
        }
        $( selector ).each(
            function() {
                var el = $( this );
                ...
                el.find( '.remove-image, .remove-file' ).unbind( 'click' ).on(
                    'click', function() {
                        avadaredux.field_objects.media.removeFile( $( this ).parents( 'fieldset.avadaredux-field:first' ) );
                    }
                );
                // Upload media button
                el.find( '.media_upload_button' ).unbind().on(
                    'click', function( event ) {
                        avadaredux.field_objects.media.addFile( event, $( this ).parents( 'fieldset.avadaredux-field:first' ) );
                    }
                );
            }
        );
    ...
    avadaredux.field_objects.media.addFile = function( event, selector ) {
        ...
        frame.on(
            'select', function() {
                ...
                if ( typeof avadaredux.field_objects.media === 'undefined' || typeof avadaredux.field_objects.media === undefined ) {
                    avadaredux.field_objects.media = {};
                }
                ...
                selector.find( '.avadaredux-background-properties' ).slideDown();
            }
        );
    ...
    avadaredux.field_objects.media.removeFile = function( selector ) {
        ...
        avadaredux_change( $( selector ).find( '.upload-id' ) );
        selector.find( '.avadaredux-background-properties' ).hide();
