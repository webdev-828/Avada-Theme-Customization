<?php
	...
	if ( ! class_exists( 'avadareduxNewsflash' ) ) {
		class avadareduxNewsflash {
			private $parent = null;
			private $notice_data = '';
			private $server_file = '';
			private $interval = 3;
			...
			public function __construct( $parent, $params ) {
				...
				$fname = AvadaRedux_Functions::bub( 'get_notice_json', $parent->args['opt_name'] );
				$mname = AvadaRedux_Functions::yo( 'display_message', $parent->args['opt_name'] );
			...
			private function display_message() {
				...
				if ( ! empty( $this->notice_data ) ) {
					...
					if ( is_array( $data ) && ! empty( $data ) ) {
						...
						if ( $data['type'] == 'avadaredux-message' ) {
							$data['type'] = 'updated avadaredux-message';
						}
