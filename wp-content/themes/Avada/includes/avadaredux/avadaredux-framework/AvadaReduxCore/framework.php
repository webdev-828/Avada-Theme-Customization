<?php
	 ...
	 * @subpackage  Core
	 * @author      AvadaRedux Framework Team
	 */
// Exit if accessed directly
	...
	if ( ! class_exists( 'AvadaReduxFrameworkInstances' ) ) {
		// Instance Container
		require_once dirname( __FILE__ ) . '/inc/class.avadaredux_instances.php';
		require_once dirname( __FILE__ ) . '/inc/lib.avadaredux_instances.php';
	}

	if ( class_exists( 'AvadaReduxFrameworkInstances' ) ) {
		add_action( 'avadaredux/init', 'AvadaReduxFrameworkInstances::get_instance' );
	}

// Don't duplicate me!
	if ( ! class_exists( 'AvadaReduxFramework' ) ) {

		// AvadaRedux CDN class
		require_once dirname( __FILE__ ) . '/inc/class.avadaredux_cdn.php';

		// AvadaRedux API class  :)
		require_once dirname( __FILE__ ) . '/inc/class.avadaredux_api.php';

		// General helper functions
		require_once dirname( __FILE__ ) . '/inc/class.avadaredux_helpers.php';
		...
		require_once dirname( __FILE__ ) . '/inc/class.avadaredux_filesystem.php';

		require_once dirname( __FILE__ ) . '/inc/class.avadaredux_admin_notices.php';

		// ThemeCheck checks
		require_once dirname( __FILE__ ) . '/inc/themecheck/class.avadaredux_themecheck.php';

		// Welcome
		require_once dirname( __FILE__ ) . '/inc/welcome/welcome.php';

		/**
		 * Main AvadaReduxFramework class
		 *
		 * @since       1.0.0
		 */
		class AvadaReduxFramework {
			...
			public static function init() {
				...
				if ( self::$_is_plugin == true || self::$_as_plugin == true ) {
					...
					} else if ( strpos( AvadaRedux_Helpers::cleanFilePath( __FILE__ ), AvadaRedux_Helpers::cleanFilePath( get_stylesheet_directory() ) ) !== false ) {
			...
			public $framework_url = 'http://www.avadareduxframework.com/';
			...
			public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
				...
				if ( ! empty ( $this->args['opt_name'] ) ) {
					 ...
					 */
					$this->sections = apply_filters( 'avadaredux-sections', $sections ); // REMOVE LATER
					/**
					 ...
					 * @param  array $sections field option sections
					 */
					$this->sections = apply_filters( "avadaredux-sections-{$this->args['opt_name']}", $this->sections ); // REMOVE LATER
					/**
					...
					if ( $this->args['database'] == "network" && $this->args['network_admin'] ) {
						add_action( 'network_admin_edit_avadaredux_' . $this->args['opt_name'], array(
							$this,
							'save_network_page'
						), 10, 0 );
...
// __construct()
			...
			private function set_avadaredux_content() {
				$upload_dir        = wp_upload_dir();
				self::$_upload_dir = $upload_dir['basedir'] . '/avadaredux/';
				self::$_upload_url = str_replace( array( 'https://', 'http://' ), '//', $upload_dir['baseurl'] . '/avadaredux/' );
			}
			...
			private function set_default_args() {
				$this->args = array(
					...
					'class'                     => '',
					// Class that gets appended to all avadaredux-containers
					'admin_bar'                 => true,
					...
					'dev_mode'                  => true,
					'templates_path'            => '',
					// Path to the templates file for various AvadaRedux elements
					'ajax_save'                 => true,
			...
			public function vc_fixes() {
				if ( avadaredux_helpers::isFieldInUse( $this, 'ace_editor' ) ) {
			...
			public function network_admin_bar( $wp_admin_bar ) {
				...
				$args = array(
					...
					'meta'   => array( 'class' => 'avadaredux-network-admin' )
				);
			...
			public function _admin_notices() {
				AvadaRedux_Admin_Notices::adminNotices( $this->admin_notices );
			}
			...
			private function _internationalization() {
				 ...
				 * filter 'avadaredux/textdomain/{opt_name}'
				 *
				 * @param string     The locale of the blog or from the 'locale' hook
				 * @param string     'avadaredux-framework'  text domain
				 */
//                $locale = apply_filters( "avadaredux/textdomain/{$this->args['opt_name']}", get_locale(), 'avadaredux-framework' );
//
...
//                }
				...
				$basepath = apply_filters( "avadaredux/textdomain/basepath/{$this->args['opt_name']}", $basepath );

				load_plugin_textdomain( 'avadaredux-framework', false, $basepath . 'languages');
			}
...
// get_instance()
			...
			private function _tracking() {
				if ( file_exists( dirname( __FILE__ ) . '/inc/tracking.php' ) ) {
					...
					$tracking = AvadaRedux_Tracking::get_instance();
					$tracking->load( $this );
				}
			}
// _tracking()
...
// set()
			...
			private function set_global_variable() {
				if ( $this->args['global_variable'] ) {
					...
					/**
					 * filter 'avadaredux/options/{opt_name}/global_variable'
					 *
					 * @param array $value option value to set global_variable with
					 */
					$GLOBALS[ $this->args['global_variable'] ] = apply_filters( "avadaredux/options/{$this->args['opt_name']}/global_variable", $this->options );
...
// set_global_variable()
			...
			public function set_options( $value = '' ) {
				...
				if ( ! empty ( $value ) ) {
					...
					//do_action( "avadaredux-saved-{$this->args['opt_name']}", $value ); // REMOVE
					//do_action( "avadaredux/options/{$this->args['opt_name']}/saved", $value, $this->transients['changed_values'] );
				}
			}
// set_options()
			...
			public function get_options() {
				...
				/**
				 * action 'avadaredux/options/{opt_name}/options'
				 *
				 * @param mixed $value option values
				 */
				$this->options = apply_filters( "avadaredux/options/{$this->args['opt_name']}/options", $this->options );
...
//return $data;
				/**
				 * filter 'avadaredux/options/{opt_name}/wordpress_data/{type}/'
				 *
				 * @deprecated
				 *
				 * @param string $data
				 */
				$data = apply_filters( "avadaredux/options/{$this->args['opt_name']}/wordpress_data/$type/", $data ); // REMOVE LATER

				/**
				 * filter 'avadaredux/options/{opt_name}/data/{type}'
				 *
				 * @param string $data
				 */
				$data = apply_filters( "avadaredux/options/{$this->args['opt_name']}/data/$type", $data );
				...
				if ( empty ( $data ) && ! empty ( $type ) ) {
					...
					if ( ! empty ( $type ) && empty ( $data ) ) {
						...
						if ( $type == "categories" || $type == "category" ) {
							...
							/**
							 * filter 'avadaredux-font-icons'
							 *
							 * @deprecated
							 *
							 * @param array $font_icons array of elusive icon classes
							 */
							$font_icons = apply_filters( 'avadaredux-font-icons', array() ); // REMOVE LATER
							 ...
							 * @param array $font_icons array of elusive icon classes
							 */
							$font_icons = apply_filters( 'avadaredux/font-icons', $font_icons );

							/**
							 * filter 'avadaredux/{opt_name}/field/font/icons'
							 *
							 * @deprecated
							 *
							 * @param array $font_icons array of elusive icon classes
							 */
							$font_icons = apply_filters( "avadaredux/{$this->args['opt_name']}/field/font/icons", $font_icons );
...
// show()
			...
			public function _default_values() {
						...
						if ( isset ( $section['fields'] ) ) {
							foreach ( $section['fields'] as $k => $field ) {
								...
								if ( $field['type'] == "section" && isset ( $field['indent'] ) && $field['indent'] == "true" ) {
									...
									$field['class'] .= " avadaredux-section-indent-start";
				...
				/**
				 * filter 'avadaredux/options/{opt_name}/defaults'
				 *
				...
				$this->options_defaults             = apply_filters( "avadaredux/options/{$this->args['opt_name']}/defaults", $this->options_defaults, $this->transients['changed_values'] );
			...
			private function _default_cleanup() {
				...
				if ( AvadaRedux_Helpers::isLocalHost() || ( AvadaRedux_Helpers::isWpDebug() ) ) {
			...
			public function _admin_bar_menu() {
				...
				if ( $menu ) {
					...
					if ( isset ( $submenu[ $this->args["page_slug"] ] ) && is_array( $submenu[ $this->args["page_slug"] ] ) ) {
						foreach ( $submenu[ $this->args["page_slug"] ] as $index => $avadaredux_options_submenu ) {
							$subnodeargs = array(
								'id'     => $this->args["page_slug"] . '_' . $index,
								'title'  => $avadaredux_options_submenu[0],
								'parent' => $this->args["page_slug"],
								'href'   => admin_url( 'admin.php?page=' . $avadaredux_options_submenu[2] ),
							);
					...
					$nodeargs = array(
						...
						'title' => $title,
						// $theme_data->get( 'Name' ) . " " . __( 'Options', 'avadaredux-framework-demo' ),
...
// _admin_bar_menu()
			...
			public function _enqueue_output() {
					...
					if ( isset ( $section['fields'] ) ) {
						...
						foreach ( $section['fields'] as $fieldk => $field ) {
							if ( isset ( $field['type'] ) && $field['type'] != "callback" ) {
								$field_class = "AvadaReduxFramework_{$field['type']}";
								if ( ! class_exists( $field_class ) ) {
									...
									/**
									 * Field class file
									 * filter 'avadaredux/{opt_name}/field/class/{field.type}
									 *
									 * @param       string        field class file
									 * @param array $field        field config data
									 */
									$class_file = apply_filters( "avadaredux/{$this->args['opt_name']}/field/class/{$field['type']}", self::$_dir . "inc/fields/{$field['type']}/field_{$field['type']}.php", $field );
								...
								if ( ! empty ( $this->options[ $field['id'] ] ) && class_exists( $field_class ) && method_exists( $field_class, 'output' ) && $this->_can_output_css( $field ) ) {
									$field = apply_filters( "avadaredux/field/{$this->args['opt_name']}/output_css", $field );
				...
				if ( ! empty ( $this->typography ) && ! empty ( $this->typography ) && filter_var( $this->args['output'], FILTER_VALIDATE_BOOLEAN ) ) {
					...
					$typography = new AvadaReduxFramework_typography ( null, null, $this );
					...
					if ( $this->args['async_typography'] && ! empty ( $this->typography ) ) {
						...
						wp_register_style( 'avadaredux-google-fonts-' . $this->args['opt_name'], $protocol . $typography->makeGoogleWebfontLink( $this->typography ), '', $version );
						wp_enqueue_style( 'avadaredux-google-fonts-' . $this->args['opt_name'] );
					}
				}
			}
// _enqueue_output()
			...
			public function _enqueue() {
				...
				$enqueue = new avadareduxCoreEnqueue ( $this );
				$enqueue->init();
			}
// _enqueue()
			...
			public function _load_page() {
				...
				if ( true == $this->show_hints ) {
					...
					$dismiss = 'true';
					$s       = __( 'Enable', 'avadaredux-framework' );

					// Values for disabling hints.
					if ( 'true' == $hint_status ) {
						$dismiss = 'false';
						$s       = __( 'Disable', 'avadaredux-framework' );
					}

					// Make URL
					$url = '<a class="avadaredux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

					$event = __( 'moving the mouse over', 'avadaredux-framework' );
					if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
						$event = __( 'clicking', 'avadaredux-framework' );
					}

					// Construct message
					$msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'avadaredux-framework' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

					// Construct hint tab
					$tab = array(
						'id'      => 'avadaredux-hint-tab',
						'title'   => __( 'Hints', 'avadaredux-framework' ),
						'content' => '<p>' . $msg . '</p>'
					);
				...
				if ( $this->args['help_sidebar'] != '' ) {
					...
					// about hints.
					if ( true == $this->show_hints ) {
						$screen->set_help_sidebar( '<p><strong>AvadaRedux Framework</strong><br/><br/>Hint Tooltip Preferences</p>' );
					}
				}

				/**
				 * action 'avadaredux-load-page-{opt_name}'
				 *
				 * @deprecated
				 *
				 * @param object $screen WP_Screen
				 */
				do_action( "avadaredux-load-page-{$this->args['opt_name']}", $screen ); // REMOVE

				/**
				 * action 'avadaredux/page/{opt_name}/load'
				 *
				 * @param object $screen WP_Screen
				 */
				do_action( "avadaredux/page/{$this->args['opt_name']}/load", $screen );
			}
// _load_page()

			/**
			 * Do action avadaredux-admin-head for options page
			 *
			 * @since       1.0.0
			 * @access      public
			 * @return      void
			...
			public function admin_head() {
				/**
				 * action 'avadaredux-admin-head-{opt_name}'
				 *
				 * @deprecated
				 *
				 * @param  object $this AvadaReduxFramework
				 */
				do_action( "avadaredux-admin-head-{$this->args['opt_name']}", $this ); // REMOVE

				/**
				 * action 'avadaredux/page/{opt_name}/header'
				 *
				 * @param  object $this AvadaReduxFramework
				 */
				do_action( "avadaredux/page/{$this->args['opt_name']}/header", $this );
			}
// admin_head()
...
// admin_footer_text()
			...
			private function get_default_output_string( $field ) {
				...
				if ( ! empty ( $default_output ) ) {
					$default_output = __( 'Default', 'avadaredux-framework' ) . ": " . substr( $default_output, 0, - 2 );
				}
...
// get_default_output_string()
			...
			public function get_header_html( $field ) {
				...
				if ( isset ( $field['title'] ) && isset ( $field['type'] ) && $field['type'] !== "info" && $field['type'] !== "section" ) {
					...
					if ( isset ( $field['hint'] ) && ! '' == $field['hint'] ) {
						...
						$hint = apply_filters( 'avadaredux/hints/html', $hint, $field, $this->args );
						...
						if ( 'true' == $metaVal || empty ( $metaVal ) && empty( $hint )  ) {
							...
							$hint = '<div class="avadaredux-hint-qtip" style="float:' . $this->args['hints']['icon_position'] . '; font-size: ' . $size . '; color:' . $hint_color . '; cursor: ' . $pointer . ';" qtip-title="' . $titleParam . '" qtip-content="' . $contentParam . '">&nbsp;<i class="' . ( isset( $this->args['hints']['icon'] ) ? $this->args['hints']['icon'] : '' ) . '"></i></div>';
						}
					}
				...
				if ( ! empty ( $th ) ) {
					$th = '<div class="avadaredux_field_th">' . $th . '</div>';
				}
			...
			public function _register_settings() {
				...
				foreach ( $this->sections as $k => $section ) {
					...
					/**
					 * filter 'avadaredux-section-{index}-modifier-{opt_name}'
					 *
					 * @param array $section section configuration
					 */
					$section = apply_filters( "avadaredux-section-{$k}-modifier-{$this->args['opt_name']}", $section );

					/**
					 * filter 'avadaredux/options/{opt_name}/section/{section.id}'
					 *
					 * @param array $section section configuration
					 */
					if ( isset ( $section['id'] ) ) {
						$section = apply_filters( "avadaredux/options/{$this->args['opt_name']}/section/{$section['id']}", $section );
					}
					...
					if ( isset ( $section['fields'] ) ) {
						foreach ( $section['fields'] as $fieldk => $field ) {
							...
							/**
							 * filter 'avadaredux/options/{opt_name}/field/{field.id}'
							 *
							 * @param array $field field config
							 */
							$field = apply_filters( "avadaredux/options/{$this->args['opt_name']}/field/{$field['id']}/register", $field );
							...
							/**
							 * filter 'avadaredux-field-{field.id}modifier-{opt_name}'
							 *
							 * @deprecated
							 *
							 * @param array $field field config
							 */
							$field = apply_filters( "avadaredux-field-{$field['id']}modifier-{$this->args['opt_name']}", $field ); // REMOVE LATER
							 ...
							 * @param array $field field config
							 */
							$field = apply_filters( "avadaredux/options/{$this->args['opt_name']}/field/{$field['id']}", $field );
							...
							/**
							 * action 'avadaredux/options/{opt_name}/field/field.type}/register'
							 */
							do_action( "avadaredux/options/{$this->args['opt_name']}/field/{$field['type']}/register", $field );
							...
							if ( ! $display || isset ( $this->no_panel_section[ $k ] ) ) {
								...
								if ( isset ( $field['hidden'] ) && $field['hidden'] ) {
									$field['label_for'] = 'avadaredux_hide_field';
								}
								if ( $this->args['options_api'] == true ) {
									add_settings_field(
				...
				/**
				 * action 'avadaredux-register-settings-{opt_name}'
				 *
				 * @deprecated
				 */
				do_action( "avadaredux-register-settings-{$this->args['opt_name']}" ); // REMOVE

				/**
				 * action 'avadaredux/options/{opt_name}/register'
				 *
				 * @param array option sections
				 */
				do_action( "avadaredux/options/{$this->args['opt_name']}/register", $this->sections );
				...
				if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
					 ...
					 */
					do_action( "avadaredux-compiler-{$this->args['opt_name']}", $this->options, $this->compilerCSS, $this->transients['changed_values'] ); // REMOVE

					/**
					 * action 'avadaredux/options/{opt_name}a'
					 *
					 * @param array  options
					 * @param string CSS that get sent to the compiler hook
					 */
...
// _register_settings()
			...
			private function _register_extensions() {
				...
				/**
				 * action 'avadaredux/extensions/before'
				 *
				 * @param object $this AvadaReduxFramework
				 */
				do_action( "avadaredux/extensions/before", $this );

				/**
				 * action 'avadaredux/extensions/{opt_name}/before'
				 *
				 * @param object $this AvadaReduxFramework
				 */
				do_action( "avadaredux/extensions/{$this->args['opt_name']}/before", $this );

				if ( isset( $this->old_opt_name ) ) {
					do_action( "avadaredux/extensions/{$this->old_opt_name}/before", $this );
				}
				...
				foreach ( $folders as $folder ) {
					...
					$extension_class = 'AvadaReduxFramework_Extension_' . $folder;

					/**
					 * filter 'avadaredux-extensionclass-load'
					 *
					 * @deprecated
					 *
					 ...
					 */
					$class_file = apply_filters( "avadaredux-extensionclass-load", "$path/$folder/extension_{$folder}.php", $extension_class ); // REMOVE LATER

					/**
					 * filter 'avadaredux/extension/{opt_name}/{folder}'
					 *
					 * @param        string                    extension class file path
					 * @param string $extension_class          extension class name
					 */
					$class_file = apply_filters( "avadaredux/extension/{$this->args['opt_name']}/$folder", "$path/$folder/extension_{$folder}.php", $class_file );
				...
				/**
				 * action 'avadaredux-register-extensions-{opt_name}'
				 *
				 * @deprecated
				 *
				 * @param object $this AvadaReduxFramework
				 */
				do_action( "avadaredux-register-extensions-{$this->args['opt_name']}", $this ); // REMOVE

				/**
				 * action 'avadaredux/extensions/{opt_name}'
				 *
				 * @param object $this AvadaReduxFramework
				 */
				do_action( "avadaredux/extensions/{$this->args['opt_name']}", $this );

				if ( isset( $this->old_opt_name ) && ! empty( $this->old_opt_name ) ) {
					do_action( "avadaredux/extensions/{$this->old_opt_name}", $this );
				}
			}
...
//print_r($plugin_options);
				...
				if ( isset ( $plugin_options['avadaredux-no_panel'] ) ) {
					$keys = explode( '|', $plugin_options['avadaredux-no_panel'] );
					foreach ( $keys as $key ) {
						$plugin_options[ $key ] = $this->options[ $key ];
					}
					if ( isset ( $plugin_options['avadaredux-no_panel'] ) ) {
						unset ( $plugin_options['avadaredux-no_panel'] );
					}
				}
				...
				if ( ( isset( $plugin_options['import_code'] ) && ! empty( $plugin_options['import_code'] ) ) || ( isset( $plugin_options['import_link'] ) && ! empty( $plugin_options['import_link'] ) ) ) {
					...
					if ( ! empty ( $imported_options ) && is_array( $imported_options ) && isset ( $imported_options['avadaredux-backup'] ) && $imported_options['avadaredux-backup'] == '1' ) {
						...
						/**
						 * action 'avadaredux/options/{opt_name}/import'
						 *
						 * @param  &array [&$plugin_options, avadaredux_options]
						 */
						do_action_ref_array( "avadaredux/options/{$this->args['opt_name']}/import", array(
							&$plugin_options,
							$imported_options,
							$this->transients['changed_values']
						) );

						setcookie( 'avadaredux_current_tab', '', 1, '/', $time + 1000, "/" );
						$_COOKIE['avadaredux_current_tab'] = 1;
				...
				if ( ! empty ( $plugin_options['defaults'] ) ) {
					...
					/**
					 * apply_filters 'avadaredux/validate/{opt_name}/defaults'
					 *
					 * @param  &array [ $this->options_defaults, $plugin_options]
					 */
					$plugin_options = apply_filters( "avadaredux/validate/{$this->args['opt_name']}/defaults", $this->options_defaults );

					// Section reset
					//setcookie('avadaredux-compiler-' . $this->args['opt_name'], 1, time() + 3000, '/');
					...
					//setcookie('avadaredux-compiler-' . $this->args['opt_name'], 1, time() + 1000, "/");
					//setcookie("avadaredux-saved-{$this->args['opt_name']}", 'defaults', time() + 1000, "/");
				...
				if ( ! empty ( $plugin_options['defaults-section'] ) ) {
					if ( isset ( $plugin_options['avadaredux-section'] ) && isset ( $this->sections[ $plugin_options['avadaredux-section'] ]['fields'] ) ) {
						/**
						 * apply_filters 'avadaredux/validate/{opt_name}/defaults_section'
						 *
						 * @param  &array [ $this->options_defaults, $plugin_options]
						 */
						foreach ( $this->sections[ $plugin_options['avadaredux-section'] ]['fields'] as $field ) {
						...
						$plugin_options = apply_filters( "avadaredux/validate/{$this->args['opt_name']}/defaults_section", $plugin_options );
					}
					...
					//setcookie("avadaredux-saved-{$this->args['opt_name']}", 'defaults_section', time() + 1000, "/");
					unset ( $plugin_options['defaults'], $plugin_options['defaults_section'], $plugin_options['import'], $plugin_options['import_code'], $plugin_options['import_link'], $plugin_options['compiler'], $plugin_options['avadaredux-section'] );
...
//                }
				...
				/**
				 * apply_filters 'avadaredux/validate/{opt_name}/before_validation'
				 *
				 * @param  &array [&$plugin_options, avadaredux_options]
				 */
				$plugin_options = apply_filters( "avadaredux/validate/{$this->args['opt_name']}/before_validation", $plugin_options, $this->options );
				...
				/**
				 * action 'avadaredux-validate-{opt_name}'
				 *
				 * @deprecated
				 *
				 * @param  &array [&$plugin_options, avadaredux_options]
				 */
				do_action_ref_array( "avadaredux-validate-{$this->args['opt_name']}", array(
					&$plugin_options,
					$this->options
				) ); // REMOVE
				...
				/**
				 * action 'avadaredux/options/{opt_name}/validate'
				 *
				 * @param  &array [&$plugin_options, avadaredux_options]
				 */
				do_action_ref_array( "avadaredux/options/{$this->args['opt_name']}/validate", array(
					&$plugin_options,
					$this->options,
					$this->transients['changed_values']
				) );
			...
			public function ajax_save() {
				if ( ! wp_verify_nonce( $_REQUEST['nonce'], "avadaredux_ajax_nonce" . $this->args['opt_name'] ) ) {
					echo json_encode( array(
						'status' => __( 'Invalid security credential.  Please reload the page and try again.', 'avadaredux-framework' ),
						'action' => ''
					) );
				...
				if ( ! current_user_can( $this->args['page_permissions'] ) ) {
					echo json_encode( array(
						'status' => __( 'Invalid user capability.  Please reload the page and try again.', 'avadaredux-framework' ),
						'action' => ''
					) );
				...
				if ( ! empty ( $_POST['data'] ) && ! empty ( $avadaredux->args['opt_name'] ) ) {
					...
					if ( ! empty ( $values ) ) {
						...
						try {
							...
							$return_array = array(
								'status'   => 'success',
								'options'  => $avadaredux->options,
								'errors'   => isset ( $avadaredux->localize_data['errors'] ) ? $avadaredux->localize_data['errors'] : null,
								'warnings' => isset ( $avadaredux->localize_data['warnings'] ) ? $avadaredux->localize_data['warnings'] : null,
							);
				...
				foreach ( $sections as $k => $section ) {
					if ( isset ( $section['fields'] ) ) {
						foreach ( $section['fields'] as $fkey => $field ) {
							...
							if ( isset ( $field['validate'] ) ) {
								$validate = 'AvadaRedux_Validation_' . $field['validate'];

								if ( ! class_exists( $validate ) ) {
									/**
									 * filter 'avadaredux-validateclass-load'
									 *
									 * @deprecated
									 *
									 ...
									 */
									$class_file = apply_filters( "avadaredux-validateclass-load", self::$_dir . "inc/validation/{$field['validate']}/validation_{$field['validate']}.php", $validate ); // REMOVE LATER

									/**
									 * filter 'avadaredux/validate/{opt_name}/class/{field.validate}'
									 *
									 * @param        string                validation class file path
									 * @param string $class_file           validation class file path
									 */
									$class_file = apply_filters( "avadaredux/validate/{$this->args['opt_name']}/class/{$field['validate']}", self::$_dir . "inc/validation/{$field['validate']}/validation_{$field['validate']}.php", $class_file );
			...
			public function section_menu( $k, $section, $suffix = "", $sections = array() ) {
				...
				if ( isset ( $section['type'] ) && $section['type'] == "divide" ) {
					...
					$string .= '<li id="' . esc_attr( $k . $suffix ) . '_section_group_li" class="avadaredux-group-tab-link-li' . esc_attr( $hide_section ) . esc_attr( $section['class'] ) . esc_attr( $subsectionsClass ) . '">';
					$string .= '<a href="javascript:void(0);" id="' . esc_attr( $k . $suffix ) . '_section_group_li_a" class="avadaredux-group-tab-link-a" data-key="' . esc_attr( $k ) . '" data-rel="' . esc_attr( $k . $suffix ) . '" data-css-id="' . esc_attr( $section['id'] ) . '">' . $extra_icon . $icon . '<span class="group_title">' . wp_kses_post( $section['title'] ) . '</span></a>';
					...
					if ( $subsections ) {
						...
						while ( $doLoop ) {
							...
							if ( count( $sections ) < $nextK || ! isset ( $sections[ $nextK ] ) || ! isset ( $sections[ $nextK ]['subsection'] ) || $sections[ $nextK ]['subsection'] != true ) {
								...
								$string .= '<li id="' . esc_attr( $nextK . $suffix ) . '_section_group_li" class="avadaredux-group-tab-link-li ' . esc_attr( $hide_sub ) . esc_attr( $section[ $nextK ]['class'] ) . ( $icon ? ' hasIcon' : '' ) . '">';
								$string .= '<a href="javascript:void(0);" id="' . esc_attr( $nextK . $suffix ) . '_section_group_li_a" class="avadaredux-group-tab-link-a" data-key="' . esc_attr( $nextK ) . '" data-rel="' . esc_attr( $nextK . $suffix ) . '" data-css-id="' . esc_attr( $sections[ $nextK ]['id'] ) . '">' . $icon . '<span class="group_title">' . wp_kses_post( $sections[ $nextK ]['title'] ) . '</span></a>';
								$string .= '</li>';
							}
						}
...
// section_menu()
			...
			public function generate_panel() {
				require_once 'core/panel.php';
				$panel = new avadareduxCorePanel ( $this );
				$panel->init();
				$this->set_transients();
			}
			...
			public function _section_desc( $section ) {
				...
				if ( isset ( $this->sections[ $id ]['desc'] ) && ! empty ( $this->sections[ $id ]['desc'] ) ) {
					echo '<div class="avadaredux-section-desc">' . $this->sections[ $id ]['desc'] . '</div>';
				}
			}
			...
			public function _field_input( $field, $v = null ) {
				...
				if ( isset ( $field['callback'] ) && ( is_callable( $field['callback'] ) || ( is_string( $field['callback'] ) && function_exists( $field['callback'] ) ) ) ) {
					...
					/**
					 * action 'avadaredux-before-field-{opt_name}'
					 *
					 * @deprecated
					 *
					 ...
					 */
					do_action( "avadaredux-before-field-{$this->args['opt_name']}", $field, $value ); // REMOVE

					/**
					 * action 'avadaredux/field/{opt_name}/{field.type}/callback/before'
					 *
					 * @param array  $field field data
					 * @param string $value field.id
					 */
					do_action_ref_array( "avadaredux/field/{$this->args['opt_name']}/{$field['type']}/callback/before", array(
						&$field,
						&$value
					) );

					/**
					 * action 'avadaredux/field/{opt_name}/callback/before'
					 *
					 * @param array  $field field data
					 * @param string $value field.id
					 */
					do_action_ref_array( "avadaredux/field/{$this->args['opt_name']}/callback/before", array(
						&$field,
						&$value
					) );
					...
					/**
					 * action 'avadaredux-after-field-{opt_name}'
					 *
					 * @deprecated
					 *
					 * @param array  $field field data
					 * @param string $value field.id
					 */
					do_action( "avadaredux-after-field-{$this->args['opt_name']}", $field, $value ); // REMOVE

					/**
					 * action 'avadaredux/field/{opt_name}/{field.type}/callback/after'
					 *
					 * @param array  $field field data
					 * @param string $value field.id
					 */
					do_action_ref_array( "avadaredux/field/{$this->args['opt_name']}/{$field['type']}/callback/after", array(
						&$field,
						&$value
					) );

					/**
					 * action 'avadaredux/field/{opt_name}/callback/after'
					 *
					 * @param array  $field field data
					 * @param string $value field.id
					 */
					do_action_ref_array( "avadaredux/field/{$this->args['opt_name']}/callback/after", array(
						&$field,
						&$value
					) );
				...
				if ( isset ( $field['type'] ) ) {
					...
					if ( ! class_exists( $field_class ) ) {
//                    $class_file = apply_filters( 'avadaredux/field/class/'.$field['type'], self::$_dir . 'inc/fields/' . $field['type'] . '/field_' . $field['type'] . '.php', $field ); // REMOVE
						/**
						 * filter 'avadaredux/{opt_name}/field/class/{field.type}'
						 *
					...
					if ( class_exists( $field_class ) ) {
						...
						/**
						 * action 'avadaredux/field/{opt_name}/{field.type}/render/before'
						 *
						 * @param array  $field field data
						 * @param string $value field id
						 */
						do_action_ref_array( "avadaredux/field/{$this->args['opt_name']}/{$field['type']}/render/before", array(
							&$field,
							&$value
						) );

						/**
						 * action 'avadaredux/field/{$this->args['opt_name']}/render/before'
						 *
						 * @param array  $field field data
						 * @param string $value field id
						 */
						do_action_ref_array( "avadaredux/field/{$this->args['opt_name']}/render/before", array(
							&$field,
							&$value
						) );
					 ...
					 */
						...
						/**
						 * filter 'avadaredux-field-{opt_name}'
						 *
						 * @deprecated
						 *
						 ...
						 * @param array $field        field data
						 */
						$_render = apply_filters( "avadaredux-field-{$this->args['opt_name']}", ob_get_contents(), $field ); // REMOVE

						/**
						 * filter 'avadaredux/field/{opt_name}/{field.type}/render/after'
						 *
						 * @param       string        rendered field markup
						 * @param array $field        field data
						 */
						$_render = apply_filters( "avadaredux/field/{$this->args['opt_name']}/{$field['type']}/render/after", $_render, $field );

						/**
						 * filter 'avadaredux/field/{opt_name}/render/after'
						 *
						 * @param       string        rendered field markup
						 * @param array $field        field data
						 */
						$_render = apply_filters( "avadaredux/field/{$this->args['opt_name']}/render/after", $_render, $field );
						...
						/**
						 * action 'avadaredux/field/{opt_name}/{field.type}/fieldset/before/{opt_name}'
						 *
						 * @param array  $field field data
						 * @param string $value field id
						 */
						do_action_ref_array( "avadaredux/field/{$this->args['opt_name']}/{$field['type']}/fieldset/before/{$this->args['opt_name']}", array(
							&$field,
							&$value
						) );

						/**
						 * action 'avadaredux/field/{opt_name}/fieldset/before/{opt_name}'
						 *
						 * @param array  $field field data
						 * @param string $value field id
						 */
						do_action_ref_array( "avadaredux/field/{$this->args['opt_name']}/fieldset/before/{$this->args['opt_name']}", array(
							&$field,
							&$value
						) );
						...
						if ( isset( $field['full_width'] ) && $field['full_width'] == true ) {
							$class_string .= "avadaredux_remove_th";
						}
						...
						echo '<fieldset id="' . $this->args['opt_name'] . '-' . $field['id'] . '" class="' . $hidden . 'avadaredux-field-container avadaredux-field avadaredux-field-init avadaredux-container-' . $field['type'] . ' ' . $class_string . '" data-id="' . $field['id'] . '" ' . $data_string . ' data-type="' . $field['type'] . '">';
						//}
						...
						/**
						 * action 'avadaredux/field/{opt_name}/{field.type}/fieldset/after/{opt_name}'
						 *
						 * @param array  $field field data
						 * @param string $value field id
						 */
						do_action_ref_array( "avadaredux/field/{$this->args['opt_name']}/{$field['type']}/fieldset/after/{$this->args['opt_name']}", array(
							&$field,
							&$value
						) );

						/**
						 * action 'avadaredux/field/{opt_name}/fieldset/after/{opt_name}'
						 *
						 * @param array  $field field data
						 * @param string $value field id
						 */
						do_action_ref_array( "avadaredux/field/{$this->args['opt_name']}/fieldset/after/{$this->args['opt_name']}", array(
							&$field,
							&$value
						) );
					}
...
// _field_input()
			...
			public function _can_output_css( $field ) {
				...
				$field = apply_filters( "avadaredux/field/{$this->args['opt_name']}/_can_output_css", $field );
...
//                        }
			 ...
			 * @return  array $result
			 */
			function avadaredux_parse_str( $string ) {
				if ( '' == $string ) {
					return false;
				}
				...
				foreach ( $pairs as $key => $pair ) {
					...
					if ( ! isset( $result[ $k ] ) ) {
						...
						$result[ $k ] = $this->avadaredux_array_merge_recursive_distinct( $result[ $k ], $params[ $k ] );
					}
				}
			 ...
			 * @return  array $merged
			 */
			function avadaredux_array_merge_recursive_distinct( array $array1, array $array2 ) {
				$merged = $array1;

				foreach ( $array2 as $key => $value ) {
					if ( is_array( $value ) && isset( $merged[$key] ) && is_array( $merged[$key] ) ) {
						$merged[$key] = $this->avadaredux_array_merge_recursive_distinct( $merged[$key], $value );
		...
		/**
		 * action 'avadaredux/init'
		 *
		 * @param null
		 */
		do_action( 'avadaredux/init', AvadaReduxFramework::init() );
	} // class_exists('AvadaReduxFramework')
