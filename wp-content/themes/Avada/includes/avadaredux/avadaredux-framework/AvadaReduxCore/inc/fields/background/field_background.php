...
// Don't duplicate me!
	if ( ! class_exists( 'AvadaReduxFramework_background' ) ) {

		/**
		 * Main AvadaReduxFramework_background class
		 *
		 * @since       3.1.5
		 */
		class AvadaReduxFramework_background {
			...
			public function render() {
				...
				if ( $this->field['background-color'] === true ) {
					...
					echo '<input data-id="' . $this->field['id'] . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '[background-color]" id="' . $this->field['id'] . '-color" class="avadaredux-color avadaredux-background-input avadaredux-color-init ' . $this->field['class'] . '"  type="text" value="' . $this->value['background-color'] . '"  data-default-color="' . ( isset( $this->field['default']['background-color'] ) ? $this->field['default']['background-color'] : "" ) . '" />';
					...
					if ( ! isset( $this->field['transparent'] ) || $this->field['transparent'] !== false ) {
						...
						echo '<label for="' . $this->field['id'] . '-transparency" class="color-transparency-check"><input type="checkbox" class="checkbox color-transparency avadaredux-background-input ' . $this->field['class'] . '" id="' . $this->field['id'] . '-transparency" data-id="' . $this->field['id'] . '-color" value="1"' . $tChecked . '> ' . __( 'Transparent', 'avadaredux-framework' ) . '</label>';
					}
				...
				if ( $this->field['background-repeat'] === true ) {
					...
					echo '<select id="' . $this->field['id'] . '-repeat-select" data-placeholder="' . __( 'Background Repeat', 'avadaredux-framework' ) . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '[background-repeat]" class="avadaredux-select-item avadaredux-background-input avadaredux-background-repeat ' . $this->field['class'] . '">';
					echo '<option></option>';
				...
				if ( $this->field['background-clip'] === true ) {
					...
					echo '<select id="' . $this->field['id'] . '-repeat-select" data-placeholder="' . __( 'Background Clip', 'avadaredux-framework' ) . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '[background-clip]" class="avadaredux-select-item avadaredux-background-input avadaredux-background-clip ' . $this->field['class'] . '">';
					echo '<option></option>';
				...
				if ( $this->field['background-origin'] === true ) {
					...
					echo '<select id="' . $this->field['id'] . '-repeat-select" data-placeholder="' . __( 'Background Origin', 'avadaredux-framework' ) . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '[background-origin]" class="avadaredux-select-item avadaredux-background-input avadaredux-background-origin ' . $this->field['class'] . '">';
					echo '<option></option>';
				...
				if ( $this->field['background-size'] === true ) {
					...
					echo '<select id="' . $this->field['id'] . '-repeat-select" data-placeholder="' . __( 'Background Size', 'avadaredux-framework' ) . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '[background-size]" class="avadaredux-select-item avadaredux-background-input avadaredux-background-size ' . $this->field['class'] . '">';
					echo '<option></option>';
				...
				if ( $this->field['background-attachment'] === true ) {
					...
					echo '<select id="' . $this->field['id'] . '-attachment-select" data-placeholder="' . __( 'Background Attachment', 'avadaredux-framework' ) . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '[background-attachment]" class="avadaredux-select-item avadaredux-background-input avadaredux-background-attachment ' . $this->field['class'] . '">';
					echo '<option></option>';
					foreach ( $array as $k => $v ) {
				...
				if ( $this->field['background-position'] === true ) {
					...
					echo '<select id="' . $this->field['id'] . '-position-select" data-placeholder="' . __( 'Background Position', 'avadaredux-framework' ) . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '[background-position]" class="avadaredux-select-item avadaredux-background-input avadaredux-background-position ' . $this->field['class'] . '">';
					echo '<option></option>';
				...
				if ( $this->field['background-image'] === true ) {
					...
					$placeholder = isset( $this->field['placeholder'] ) ? $this->field['placeholder'] : __( 'No media selected', 'avadaredux-framework' );

					echo '<input placeholder="' . $placeholder . '" type="text" class="avadaredux-background-input ' . $hide . 'upload ' . $this->field['class'] . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '[background-image]" id="' . $this->parent->args['opt_name'] . '[' . $this->field['id'] . '][background-image]" value="' . $this->value['background-image'] . '" />';
					...
					echo '<img class="avadaredux-option-image" id="image_' . $this->value['media']['id'] . '" src="' . $this->value['media']['thumbnail'] . '" alt="" target="_blank" rel="external" />';
					echo '</a>';
					echo '</div>';
					...
					echo '<span class="button avadaredux-background-upload" id="' . $this->field['id'] . '-media">' . __( 'Upload', 'avadaredux-framework' ) . '</span>';
					...
					echo '<span class="button removeCSS avadaredux-remove-background' . $hide . '" id="reset_' . $this->field['id'] . '" rel="' . $this->field['id'] . '">' . __( 'Remove', 'avadaredux-framework' ) . '</span>';
			...
			public function enqueue() {
				...
				if (!wp_script_is ( 'avadaredux-field-background-js' )) {
					wp_enqueue_script(
						'avadaredux-field-background-js',
						AvadaReduxFramework::$_url . 'inc/fields/background/field_background' . AvadaRedux_Functions::isMin() . '.js',
				...
				if ($this->parent->args['dev_mode']) {
					if (!wp_style_is ( 'avadaredux-field-background-css' )) {
						wp_enqueue_style(
							'avadaredux-field-background-css',
							AvadaReduxFramework::$_url . 'inc/fields/background/field_background.css',
							array(),
							time(),
							'all'
						);
