...
(function( $ ) {  //This functions first parameter is named $
    ...
    avadaredux.customizer = avadaredux.customizer || {};

    $( document ).ready(
        function() {
            avadaredux.customizer.init();
        }
    );
    avadaredux.customizer.init = function() {
        $( 'body' ).addClass( avadaredux_customizer.body_class );
        $( '.accordion-section.avadaredux-section, .accordion-section.avadaredux-panel, .accordion-section-title' ).click(
            function() {
                $.avadaredux.initFields();
            }
        );
    };
    avadaredux.customizer.save = function( $obj, $container ) {
        var $parent = $obj.hasClass( 'avadaredux-field' ) ? $obj : $obj.parents( '.avadaredux-field-container:first' );
        var $id = $parent.parent().find( '.avadaredux-customizer-input' ).data( 'id' );
