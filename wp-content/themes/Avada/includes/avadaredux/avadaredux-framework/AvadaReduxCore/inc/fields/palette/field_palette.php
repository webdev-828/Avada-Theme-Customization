...
if ( ! class_exists( 'AvadaReduxFramework_palette' ) ) {
	class AvadaReduxFramework_palette {
		...
		public function render() {
			...
			foreach ( $this->field['palettes'] as $value => $colorSet ) {
				...
				echo '<input type="radio" value="' . $value . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '" class="avadaredux-palette-set ' . $this->field['class'] . '" id="' . $this->field['id'] . '-' . $value . '"' . $checked . '>';
		...
		public function enqueue() {
			...
			wp_enqueue_script(
				'avadaredux-field-palette-js',
				AvadaReduxFramework::$_url . 'inc/fields/palette/field_palette' . $min . '.js',
				array( 'jquery', 'avadaredux-js', 'jquery-ui-button', 'jquery-ui-core' ),
				time(),
				true
			);
			...
			if ($this->parent->args['dev_mode']) {
				wp_enqueue_style(
					'avadaredux-field-palette-css',
					AvadaReduxFramework::$_url . 'inc/fields/palette/field_palette.css',
					array(),
					time(),
					'all'
				);
