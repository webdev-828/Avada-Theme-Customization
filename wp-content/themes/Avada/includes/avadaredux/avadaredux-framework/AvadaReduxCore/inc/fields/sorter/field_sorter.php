...
// Exit if accessed directly
	...
	if ( ! class_exists( 'AvadaReduxFramework_sorter' ) ) {
		class AvadaReduxFramework_sorter {

			/**
			 * Field Constructor.
			 * Required - must call the parent constructor, then assign field and value to vars, and obviously call the render field function
			 *
			 * @since AvadaRedux_Options 1.0.0
			 */
			...
			function render() {
				...
				if ( is_array( $sortlists ) ) {
					...
					if ( $sortlists ) {
						echo '<fieldset id="' . esc_attr($this->field['id']) . '" class="avadaredux-sorter-container avadaredux-sorter">';
			...
			function enqueue() {
				if ( $this->parent->args['dev_mode'] ) {
					wp_enqueue_style(
						'avadaredux-field-sorder-css',
						AvadaReduxFramework::$_url . 'inc/fields/sorter/field_sorter.css',
						array(),
						time(),
						'all'
					);
				...
				wp_enqueue_script(
					'avadaredux-field-sorter-js',
					AvadaReduxFramework::$_url . 'inc/fields/sorter/field_sorter' . AvadaRedux_Functions::isMin() . '.js',
					array( 'jquery', 'avadaredux-js', 'jquery-ui-sortable' ),
					time(),
					true
				);
			}
