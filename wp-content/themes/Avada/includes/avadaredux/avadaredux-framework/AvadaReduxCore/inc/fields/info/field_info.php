...
// Don't duplicate me!
	if ( ! class_exists( 'AvadaReduxFramework_info' ) ) {

		/**
		 * Main AvadaReduxFramework_info class
		 *
		 * @since       1.0.0
		 */
		class AvadaReduxFramework_info {
			...
			public function render() {
				...
				if ( empty( $this->field['raw_html'] ) ) {
					if ( $this->field['notice'] == true ) {
						$this->field['class'] .= ' avadaredux-notice-field';
					} else {
						$this->field['class'] .= ' avadaredux-info-field';
					}



					$this->field['style'] = 'avadaredux-' . $this->field['style'] . ' ';
				}

				$indent = ( isset( $this->field['sectionIndent'] ) && $this->field['sectionIndent'] ) ? ' form-table-section-indented' : '';

				echo '</td></tr></table><div id="info-' . esc_attr($this->field['id']) . '" class="' . ( isset( $this->field['icon'] ) && ! empty( $this->field['icon'] ) && $this->field['icon'] !== true ? "hasIcon " : "") . esc_attr($this->field['style']) . ' ' . esc_attr($this->field['class']) . ' avadaredux-field-' . esc_attr($this->field['type']) . esc_attr($indent) . '"'.( !empty($this->field['color']) ? ' style="' . esc_attr($this->field['color']) . '"' : '' ) . '>';
				...
				if ( ! empty( $this->field['raw_html'] ) && $this->field['raw_html'] ) {
					...
					if ( isset( $this->field['icon'] ) && ! empty( $this->field['icon'] ) && $this->field['icon'] !== true ) {
						echo '<p class="avadaredux-info-icon"><i class="' . esc_attr($this->field['icon']) . ' icon-large"></i></p>';
					}
					...
					if ( ! empty( $this->field['title'] ) || ! empty( $this->field['desc'] ) ) {
						echo '<p class="avadaredux-info-desc">' . wp_kses_post($this->field['title']) . wp_kses_post($this->field['desc']) . '</p>';
					}
				}
			...
			public function enqueue() {
				if ($this->parent->args['dev_mode']) {
					wp_enqueue_style(
						'avadaredux-field-info-css',
						AvadaReduxFramework::$_url . 'inc/fields/info/field_info.css',
						array(),
						time(),
						'all'
					);
