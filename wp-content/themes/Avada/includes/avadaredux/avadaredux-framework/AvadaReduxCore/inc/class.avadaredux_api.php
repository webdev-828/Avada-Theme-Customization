<?php
	...
	/**
	 * AvadaRedux Framework API Class
	 * Makes instantiating a AvadaRedux object an absolute piece of cake.
	 *
	...
	if ( ! class_exists( 'AvadaRedux' ) ) {
		 ...
		 * Simple API for AvadaRedux Framework
		 *
		 * @since       1.0.0
		 */
		class AvadaRedux {
			...
			public static function load() {
				add_action( 'after_setup_theme', array( 'AvadaRedux', 'createAvadaRedux' ) );
				add_action( 'init', array( 'AvadaRedux', 'createAvadaRedux' ) );
				add_action( 'switch_theme', array( 'AvadaRedux', 'createAvadaRedux' ) );
			}

			public static function init( $opt_name = "" ) {
				if ( ! empty( $opt_name ) ) {
					self::loadAvadaRedux( $opt_name );
					remove_action( 'setup_theme', array( 'AvadaRedux', 'createAvadaRedux' ) );
				}
			}

			public static function loadExtensions( $AvadaReduxFramework ) {
				if ( $instanceExtensions = self::getExtensions( $AvadaReduxFramework->args['opt_name'], "" ) ) {
					foreach ( $instanceExtensions as $name => $extension ) {
						if ( ! class_exists( $extension['class'] ) ) {
							// In case you wanted override your override, hah.
							$extension['path'] = apply_filters( 'avadaredux/extension/' . $AvadaReduxFramework->args['opt_name'] . '/' . $name, $extension['path'] );
							if ( file_exists( $extension['path'] ) ) {
								require_once $extension['path'];
							}
						}
						if ( ! isset( $AvadaReduxFramework->extensions[ $name ] ) ) {
							if ( class_exists( $extension['class'] ) ) {
								$AvadaReduxFramework->extensions[ $name ] = new $extension['class']( $AvadaReduxFramework );
							} else {
			...
			public static function extensionPath( $extension, $folder = true ) {
				if ( ! isset( AvadaRedux::$extensions[ $extension ] ) ) {
					return;
				}
				$path = end( AvadaRedux::$extensions[ $extension ] );
				if ( ! $folder ) {
					return $path;
				}
			...
			public static function loadAvadaRedux( $opt_name = "" ) {

				if ( empty( $opt_name ) ) {
					return;
				}

				$check = AvadaReduxFrameworkInstances::get_instance( $opt_name );
				...
				if ( ! class_exists( 'AvadaReduxFramework' ) ) {
					echo '<div id="message" class="error"><p>AvadaRedux Framework is <strong>not installed</strong>. Please install it.</p></div>';

					return;
				}
				if ( isset( self::$uses_extensions[ $opt_name ] ) && ! empty( self::$uses_extensions[ $opt_name ] ) ) {
					add_action( "avadaredux/extensions/{$opt_name}/before", array( 'AvadaRedux', 'loadExtensions' ), 0 );
				}

				$avadaredux                   = new AvadaReduxFramework( $sections, $args );
				$avadaredux->apiHasRun        = 1;
				self::$init[ $opt_name ] = 1;
				if ( isset( $avadaredux->args['opt_name'] ) && $avadaredux->args['opt_name'] != $opt_name ) {
					self::$init[ $avadaredux->args['opt_name'] ] = 1;
				}

			}

			public static function createAvadaRedux() {
			...
			public static function setSections( $opt_name = '', $sections = array() ) {
				...
				if ( ! empty( $sections ) ) {
					foreach ( $sections as $section ) {
						AvadaRedux::setSection( $opt_name, $section );
					}
				}
			}
			...
			public static function getOption ($opt_name = "", $key = "") {
				...
				if (!empty($opt_name) && !empty($key)) {
					$avadaredux = get_option($opt_name);

					if (isset($avadaredux[$key])) {
						return $avadaredux[$key];
					} else {
						return;
					}
				} else {
			...
			public static function setOption ($opt_name = "", $key = "", $option = "") {
				...
				if (!empty($opt_name) && !empty($key)) {
					$avadaredux          = get_option($opt_name);
					$avadaredux[$key]    = $option;

					return update_option($opt_name, $avadaredux);
				} else {
					return false;
				}
			}
			...
			public static function checkExtensionClassFile( $opt_name, $name = "", $class_file = "", $instance = "" ) {
				if ( file_exists( $class_file ) ) {
					...
					$version                   = AvadaRedux_Helpers::get_template_version( $class_file );
			...
			public static function getAllExtensions() {
				...
				foreach ( $avadaredux as $instance ) {
					if ( ! empty( self::$uses_extensions[ $instance['args']['opt_name'] ] ) ) {
						continue;
					}
					if ( ! empty( $instance['extensions'] ) ) {

						AvadaRedux::getInstanceExtensions( $instance['args']['opt_name'], $instance );
					}
				}
			}
