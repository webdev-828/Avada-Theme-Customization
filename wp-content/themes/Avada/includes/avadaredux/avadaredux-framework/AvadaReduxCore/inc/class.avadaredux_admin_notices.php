<?php
	...
	/**
	 * AvadaRedux Framework Admin Notice Class
	...
	// Don't duplicate me!
	if ( ! class_exists( 'AvadaRedux_Admin_Notices' ) ) {
		 ...
		 * @since       1.0.0
		 */
		class AvadaRedux_Admin_Notices {

			static public $_parent;

			public static function load() {
				add_action( 'wp_ajax_avadaredux_hide_admin_notice', array(
					'AvadaRedux_Admin_Notices',
			...
			public static function adminNotices( $notices = array() ) {
				...
				if ( ! empty( $notices ) ) {
					...
					foreach ( $notices as $notice ) {
						...
						$add_style = '';
						if ( strpos( $notice['type'], 'avadaredux-message' ) != false ) {
						...
						if ( true == $notice['dismiss'] ) {
							...
							if ( ! get_user_meta( $userid, 'ignore_' . $notice['id'] ) ) {
								...
								if ( $pagenow == 'admin.php' || $pagenow == 'themes.php' ) {
									...
									// the avadaredux page slug if the GET is empty.
								...
								if ( version_compare( $wp_version, '4.2', '>' ) ) {
									...
									$css_class = esc_attr( $notice['type'] ) . ' avadaredux-notice notice is-dismissible avadaredux-notice';
									...
									echo '<div ' . $add_style . ' class="' . esc_attr( $notice['type'] ) . ' notice is-dismissable"><p>' . wp_kses_post( $notice['msg'] ) . '&nbsp;&nbsp;<a href="?dismiss=true&amp;id=' . esc_attr( $notice['id'] ) . $pageName . $curTab . '">' . esc_html__( 'Dismiss', 'avadaredux-framework' ) . '</a>.</p></div>';
								}
							}
						} else {
							// Standard notice
						...
						<script>
							jQuery( document ).ready(
								function( $ ) {
									$( 'body' ).on(
										'click', '.avadaredux-notice.is-dismissible .notice-dismiss', function( event ) {
											var $data = $( this ).parent().find( '.dismiss_data' );
											$.post(
												ajaxurl, {
													action: 'avadaredux_hide_admin_notice',
													id: $data.attr( 'id' ),
													nonce: $data.val()
												}
											);
