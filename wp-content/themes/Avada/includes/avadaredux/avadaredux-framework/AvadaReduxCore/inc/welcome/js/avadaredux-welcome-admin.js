(function( $ ) {
    ...
    $.avadaredux_welcome = $.avadaredux_welcome || {};

    $( document ).ready(
        function() {
            $.avadaredux_welcome.initQtip();
            if ( jQuery( document.getElementById( "support_div" ) ).is( ":visible" ) ) {
                $.avadaredux_welcome.initSupportPage();
            }
            $.avadaredux_welcome.supportHash();
        }
    );


    $.avadaredux_welcome.supportHash = function() {
        ...
        jQuery( '.avadaredux_support_hash' ).click(
            function( e ) {
                ...
                var $nonce = jQuery( '#avadaredux_support_nonce' ).val();
                ...
                jQuery.ajax(
                    {
                        ...
                        url: ajaxurl,
                        data: {
                            action: "avadaredux_support_hash",
                            nonce: $nonce
                        },
                        ...
                        success: function( response ) {
                            if ( response.status == "success" ) {
                                jQuery( '#support_hash' ).val( 'http://support.avadaredux.io/?id=' + response.identifier );
    ...
    $.avadaredux_welcome.initSupportPage = function() {
        //jQuery time
    ...
    $.avadaredux_welcome.initQtip = function() {
        if ( $().qtip ) {
