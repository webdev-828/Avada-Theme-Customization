...
(function( $ ) {
    ...
    avadaredux.field_objects.slider = avadaredux.field_objects.slider || {};
    ...
    avadaredux.field_objects.slider.init = function( selector ) {

        if ( !selector ) {
            selector = $( document ).find( ".avadaredux-group-tab:visible" ).find( '.avadaredux-container-slider:visible' );
        }
        ...
        $( selector ).each(
            function() {
                ...
                el.find( 'div.avadaredux-slider-container' ).each(
                    function() {
                        ...
                        if ( displayValue == DISPLAY_TEXT ) {
                            defClassOne = el.find( '.avadaredux-slider-input-one-' + mainID );
                            defClassTwo = el.find( '.avadaredux-slider-input-two-' + mainID );

                            inputOne = defClassOne;
                            inputTwo = defClassTwo;
                        } else if ( displayValue == DISPLAY_SELECT ) {
                            defClassOne = el.find( '.avadaredux-slider-select-one-' + mainID );
                            defClassTwo = el.find( '.avadaredux-slider-select-two-' + mainID );

                            avadaredux.field_objects.slider.loadSelect( defClassOne, minVal, maxVal, resVal, stepVal );

                            if ( handles === 2 ) {
                                avadaredux.field_objects.slider.loadSelect( defClassTwo, minVal, maxVal, resVal, stepVal );
                            }

                        } else if ( displayValue == DISPLAY_LABEL ) {
                            defClassOne = el.find( '#avadaredux-slider-label-one-' + mainID );
                            defClassTwo = el.find( '#avadaredux-slider-label-two-' + mainID );
                        } else if ( displayValue == DISPLAY_NONE ) {
                            defClassOne = el.find( '.avadaredux-slider-value-one-' + mainID );
                            defClassTwo = el.find( '.avadaredux-slider-value-two-' + mainID );
                        }
                                ...
                                slide: function() {
                                    if ( displayValue == DISPLAY_LABEL ) {
                                        if ( handles === 2 ) {
                                            ...
                                            el.find( 'input.avadaredux-slider-value-one-' + mainID ).attr(
                                                'value', inpSliderVal[0]
                                            );
                                            el.find( 'input.avadaredux-slider-value-two-' + mainID ).attr(
                                    ...
                                    if ( displayValue == DISPLAY_SELECT ) {
                                        if ( handles === 2 ) {
                                            el.find( '.avadaredux-slider-select-one' ).select2( 'val', slider.val()[0] );
                                            el.find( '.avadaredux-slider-select-two' ).select2( 'val', slider.val()[1] );
                                        } else {
                                            el.find( '.avadaredux-slider-select-one' ).select2( 'val', slider.val() );
                                        }
                                    }

                                    avadaredux_change( $( this ).parents( '.avadaredux-field-container:first' ).find( 'input' ) );
                                }
                            }
                        );
                ...
                el.find( 'select.avadaredux-slider-select-one, select.avadaredux-slider-select-two' ).select2( default_params );
    ...
    avadaredux.field_objects.slider.isFloat = function( mixed_var ) {
        return +mixed_var === mixed_var && (!(isFinite( mixed_var ))) || Boolean( (mixed_var % 1) );
    };

    // Return number of integers after the decimal point.
    avadaredux.field_objects.slider.decimalCount = function( res ) {
        var q = res.toString().split( '.' );
        return q[1].length;
    };

    avadaredux.field_objects.slider.loadSelect = function( myClass, min, max, res, step ) {
        ...
        for ( var i = min; i <= max; i = i + res ) {
            ...
            var n = i;
            if ( avadaredux.field_objects.slider.isFloat( res ) ) {
                var decCount = avadaredux.field_objects.slider.decimalCount( res );
