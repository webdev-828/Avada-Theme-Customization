...
// Don't duplicate me!
if( !class_exists( 'AvadaReduxFramework_color_rgba' ) ) {

		/**
		 * Main AvadaReduxFramework_color_rgba class
		 *
		 * @since       1.0.0
		 */
		class AvadaReduxFramework_color_rgba {
			 ...
			 */
				...
				public function render() {
						...
						echo '<div
											class="avadaredux-color-rgba-container ' . $this->field['class'] . '"
											data-id="'                        . $field_id . '"
				...
				public function enqueue() {
						...
						// Field dependent JS
						if (!wp_script_is ( 'avadaredux-field-color-rgba-js' )) {
								wp_enqueue_script(
										'avadaredux-field-color-rgba-js',
										AvadaReduxFramework::$_url . 'inc/fields/color_rgba/field_color_rgba' . AvadaRedux_Functions::isMin() . '.js',
										array('jquery', 'avadaredux-spectrum-js'),
										time(),
										true
								);
						}
						...
						// Spectrum CSS
						if (!wp_style_is ( 'avadaredux-spectrum-css' )) {
								wp_enqueue_style('avadaredux-spectrum-css');
						}

						if ($this->parent->args['dev_mode']) {
								if (!wp_style_is ( 'avadaredux-field-color-rgba-css' )) {
										wp_enqueue_style(
												'avadaredux-field-color-rgba-css',
												AvadaReduxFramework::$_url . 'inc/fields/color_rgba/field_color_rgba.css',
												array(),
												time(),
												'all'
										);
