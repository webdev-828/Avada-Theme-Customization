<?php
	...
	if ( ! class_exists( 'AvadaRedux_Filesystem' ) ) {
		class AvadaRedux_Filesystem {
			...
			public function __construct() {
				$this->parent->admin_notices[] = array(
					'type'    => 'error',
					'msg'     => '<strong>' . __( 'File Permission Issues', 'avadaredux-framework' ) . '</strong><br/>' . sprintf( __( 'We were unable to modify required files. Please check your permissions, or modify your wp-config.php file to contain your FTP login credentials as <a href="%s" target="_blank">outlined here</a>.', 'avadaredux-framework' ), 'https://codex.wordpress.org/Editing_wp-config.php#WordPress_Upgrade_Constants' ),
					'id'      => 'avadaredux-wp-login',
					'dismiss' => false,
				);
			}
			...
			public function execute( $action, $file = '', $params = '' ) {
				...
				if ( ! is_dir( AvadaReduxFramework::$_upload_dir ) ) {
					wp_mkdir_p( AvadaReduxFramework::$_upload_dir );
				}
				...
				$url = wp_nonce_url( $base, 'avadaredux-options' );
		...
		AvadaRedux_Filesystem::get_instance();
	}
