...
// Don't duplicate me!
	if ( ! class_exists( 'AvadaReduxFramework_section' ) ) {

		/**
		 * Main AvadaReduxFramework_heading class
		 *
		 * @since       1.0.0
		 */
		class AvadaReduxFramework_section {
			...
			public function render() {
				...
				echo '<div id="section-' . esc_attr($this->field['id']) . '" class="avadaredux-section-field avadaredux-field ' . esc_attr($this->field['style']) . ' ' . esc_attr($this->field['class']) . ' ">';
				...
				if ( ! empty( $this->field['subtitle'] ) ) {
					echo '<div class="avadaredux-section-desc">' . esc_html($this->field['subtitle']) . '</div>';
				}
				...
				<script type="text/javascript">
					jQuery( document ).ready(
						function() {
							...
							var group = jQuery( '#<?php echo esc_js($this->field['id']); ?>-marker' ).parents( '.avadaredux-group-tab:first' );
							if ( !group.hasClass( 'sectionsChecked' ) ) {
								group.addClass( 'sectionsChecked' );
								var test = group.find( '.avadaredux-section-indent-start h3' );
								jQuery.each(
			...
			public function enqueue() {
				if ( $this->parent->args['dev_mode'] ) {
					wp_enqueue_style(
						'avadaredux-field-section-css',
						AvadaReduxFramework::$_url . 'inc/fields/section/field_section.css',
						array(),
						time(),
						'all'
					);
