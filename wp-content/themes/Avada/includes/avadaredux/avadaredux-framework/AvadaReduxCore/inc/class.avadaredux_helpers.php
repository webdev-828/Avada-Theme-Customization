...
// Don't duplicate me!
	if ( ! class_exists( 'AvadaRedux_Helpers' ) ) {

		/**
		 * AvadaRedux Helpers Class
		 * Class of useful functions that can/should be shared among all AvadaRedux files.
		 *
		 * @since       1.0.0
		 */
		class AvadaRedux_Helpers {
			...
			public static function tabFromField( $parent, $field ) {
				foreach ( $parent->sections as $k => $section ) {
					...
					if ( isset( $section['fields'] ) && ! empty( $section['fields'] ) ) {
						if ( AvadaRedux_Helpers::recursive_array_search( $field, $section['fields'] ) ) {
							return $k;
							continue;
						}
					}
...
//                }
			...
			public static function trackingObject() {
				...
				return AvadaRedux_Helpers::getTrackingObject();
			}
			...
			private static function avadareduxAsPlugin() {
				return AvadaReduxFramework::$_as_plugin;
			}
			 ...
			 *
			 * @since AvadaReduxFramework 3.0.4
			 */
			...
			public static function localize( $localize ) {
				$avadaredux            = AvadaReduxFrameworkInstances::get_instance( $localize['args']['opt_name'] );
				$nonce            = wp_create_nonce( 'avadaredux-ads-nonce' );
				$base             = admin_url( 'admin-ajax.php' ) . '?action=avadaredux_p&nonce=' . $nonce . '&url=';
				$localize['rAds'] = AvadaRedux_Helpers::rURL_fix( $base, $avadaredux->args['opt_name'] );
			...
			public static function compileSystemStatus( $json_output = false, $remote_checks = false ) {
				...
				if ( $remote_checks == true ) {
					$response = wp_remote_post( 'https://www.paypal.com/cgi-bin/webscr', array(
						'sslverify'  => false,
						'timeout'    => 60,
						'user-agent' => 'AvadaReduxFramework/' . AvadaReduxFramework::$_version,
						'body'       => array(
			...
			private static function getAvadaReduxTemplates( $custom_template_path ) {
				$filesystem         = AvadaRedux_Filesystem::get_instance();
				$template_paths     = array( 'AvadaReduxFramework' => AvadaReduxFramework::$_dir . 'templates/panel' );
				$scanned_files      = array();
				$found_files        = array();
				...
				foreach ( $scanned_files as $plugin_name => $files ) {
					foreach ( $files as $file ) {
						...
						if ( $theme_file ) {
							$core_version  = self::get_template_version( AvadaReduxFramework::$_dir . 'templates/panel/' . $file );
							...
							if ( $core_version && ( empty( $theme_version ) || version_compare( $theme_version, $core_version, '<' ) ) ) {
								...
								$found_files[ $plugin_name ][] = sprintf( __( '<code>%s</code> version <strong style="color:red">%s</strong> is out of date. The core version is %s', 'avadaredux-framework' ), str_replace( WP_CONTENT_DIR . '/themes/', '', $theme_file ), $theme_version ? $theme_version : '-', $core_version );
							} else {
					...
					foreach ( $files as $key => $value ) {
						if ( ! in_array( $value, array( ".", ".." ) ) ) {
							if ( is_dir( $template_path . DIRECTORY_SEPARATOR . $value ) ) {
								$sub_files = avadaredux_scan_template_files( $template_path . DIRECTORY_SEPARATOR . $value );
