<?php
	...
	/**
	 * AvadaRedux Framework CDN Container Class
	 *
...
// Exit if accessed directly
	...
	if ( ! class_exists( 'AvadaRedux_CDN' ) ) {
		class AvadaRedux_CDN {
					...
					if ( is_wp_error( $cdn_response ) || wp_remote_retrieve_response_code( $cdn_response ) != '200' ) {
						...
						if ( class_exists( 'AvadaRedux_VendorURL' ) ) {
							...
							if ( ! self::is_enqueued( $handle, 'enqueued', $is_script ) ) {
								$msg = __( 'Please wait a few minutes, then try refreshing the page. Unable to load some remotely hosted scripts.', 'avadaredux-framework' );
								if ( self::$_parent->args['dev_mode'] ) {
									$msg = sprintf( __( 'If you are developing offline, please download and install the <a href="%s" target="_blank">AvadaRedux Vendor Support</a> plugin/extension to bypass the our CDN and avoid this warning', 'avadaredux-framework' ), 'https://github.com/avadareduxframework/avadaredux-vendor-support' );
								}

								self::$_parent->admin_notices[] = array(
									'type'    => 'error',
									'msg'     => '<strong>' . __( 'AvadaRedux Framework Warning', 'avadaredux-framework' ) . '</strong><br/>' . sprintf( __( '%s CDN unavailable.  Some controls may not render properly.', 'avadaredux-framework' ), $handle ) . '  ' . $msg,
									'id'      => $handle . $tran_key,
									'dismiss' => false,
								);
