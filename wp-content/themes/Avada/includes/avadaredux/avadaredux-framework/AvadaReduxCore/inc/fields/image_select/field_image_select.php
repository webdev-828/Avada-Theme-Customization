...
// Don't duplicate me!
if ( ! class_exists( 'AvadaReduxFramework_image_select' ) ) {

	/**
	 * Main AvadaReduxFramework_image_select class
	 *
	 * @since       1.0.0
	 */
	class AvadaReduxFramework_image_select {
		...
		public function render() {
			...
			if ( ! empty( $this->field['options'] ) ) {
				echo '<div class="avadaredux-table-container">';
				echo '<ul class="avadaredux-image-select">';
				...
				foreach ( $this->field['options'] as $k => $v ) {
					...
					$selected = ( checked( $this->value, $theValue, false ) != '' ) ? ' avadaredux-image-select-selected' : '';
					...
					if ( isset( $this->field['presets'] ) && $this->field['presets'] !== false ) {
						...
						$v['presets']['avadaredux-backup'] = 1;

						$presets   = ' data-presets="' . htmlspecialchars( json_encode( $v['presets'] ), ENT_QUOTES, 'UTF-8' ) . '"';
						$is_preset = true;

						$this->field['class'] = trim( $this->field['class'] ) . ' avadaredux-presets';
					}
					...
					echo '<li class="avadaredux-image-select">';
					echo '<label class="' . $selected . ' avadaredux-image-select' . $is_preset_class . $this->field['id'] . '_' . $x . '" for="' . $this->field['id'] . '_' . ( array_search( $k, array_keys( $this->field['options'] ) ) + 1 ) . '">';
		...
		public function enqueue() {
			...
			wp_enqueue_script(
				'avadaredux-field-image-select-js',
				AvadaReduxFramework::$_url . 'inc/fields/image_select/field_image_select' . AvadaRedux_Functions::isMin() . '.js',
			...
			if ($this->parent->args['dev_mode']) {
				wp_enqueue_style(
					'avadaredux-field-image-select-css',
					AvadaReduxFramework::$_url . 'inc/fields/image_select/field_image_select.css',
					array(),
					time(),
					'all'
				);
