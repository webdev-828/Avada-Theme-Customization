<?php
	// <input type="radio" value="1" name="_customize-radio-avadaredux_demo[opt-radio]" data-customize-setting-link="avadaredux_demo[opt-color-title]">
//return;
	/**
...
// Don't duplicate me!
	if ( ! class_exists( 'AvadaReduxFramework_extension_customizer' ) ) {
		...
		/**
		 * Main AvadaReduxFramework customizer extension class
		 *
		 * @since       1.0.0
		 */
		class AvadaReduxFramework_extension_customizer {

			// Protected vars
			protected $avadaredux;
			...
			public function __construct( $parent ) {
				...
				add_filter( "avadaredux/extension/{$this->parent->args['opt_name']}/customizer", array(
					$this,
				...
				if ( isset( $_POST['wp_customize'] ) && $_POST['wp_customize'] == "on" && isset( $_POST['customized'] ) && ! empty( $_POST['customized'] ) && ! isset( $_POST['action'] ) ) {
					add_action( "avadaredux/options/{$this->parent->args['opt_name']}/options", array(
						$this,
						'_override_values'
					), 100 );
				}
			...
			function enqueue_controls_css() {
				...
				wp_enqueue_script(
					'avadaredux-extension-customizer',
					$this->_extension_url . 'extension_customizer' . AvadaRedux_Functions::isMin() . '.js',
					array( 'jquery', 'avadaredux-js' ),
					AvadaReduxFramework_extension_customizer::$version,
					true
				);
			...
			function customize_preview_init() {
				do_action( 'avadaredux/customizer/live_preview' );
			}
			...
			public function _enqueue_new() {
				...
				/*
				  wp_enqueue_script('avadaredux-extension-customizer-codemirror-js', $this->_extension_url . 'new/codemirror.min.js');
				  wp_enqueue_script('avadaredux-extension-customizer-color-js', $this->_extension_url . 'new/colors-control.js');
				  wp_enqueue_script('avadaredux-extension-customizer-controls-js', $this->_extension_url . 'new/customizer-control.js');
				  wp_enqueue_script('avadaredux-extension-customizer-fonts-js', $this->_extension_url . 'new/fonts-customizer-admin.js');
				  wp_enqueue_script('avadaredux-extension-customizer-header-js', $this->_extension_url . 'new/header-control.js');
				  wp_enqueue_script('avadaredux-extension-customizer-models-js', $this->_extension_url . 'new/header-models.js');
				  wp_enqueue_script('avadaredux-extension-customizer-slimscroll-js', $this->_extension_url . 'new/jquery.slimscroll.js');
				  wp_enqueue_script('avadaredux-extension-customizer-droppable-js', $this->_extension_url . 'new/jquery.ui.droppable.min.js');
				  wp_enqueue_script('avadaredux-extension-customizer-editor-js', $this->_extension_url . 'new/media-editor.min.js');
				  wp_enqueue_script('avadaredux-extension-customizer-new-js', $this->_extension_url . 'new/new-customizer.js');
				  wp_enqueue_script('avadaredux-extension-customizer-previewing-js', $this->_extension_url . 'new/previewing.js');
				  wp_enqueue_script('avadaredux-extension-customizer-theme-js', $this->_extension_url . 'new/theme-customizer.js');
				 */
			}

			public function render( $control ) {
				$fieldID = str_replace( $this->parent->args['opt_name'] . '-', '', $control->avadaredux_id );
			...
			public function _register_customizer_controls( $wp_customize ) {
				...
				if ( ! class_exists( 'AvadaRedux_Customizer_Section' ) ) {
					require_once dirname( __FILE__ ) . '/inc/customizer_section.php';
					if ( method_exists( $wp_customize, 'register_section_type' ) ) {
						$wp_customize->register_section_type( 'AvadaRedux_Customizer_Section' );
					}
				}
				if ( ! class_exists( 'AvadaRedux_Customizer_Panel' ) ) {
					require_once dirname( __FILE__ ) . '/inc/customizer_panel.php';
					if ( method_exists( $wp_customize, 'register_panel_type' ) ) {
						$wp_customize->register_panel_type( 'AvadaRedux_Customizer_Panel' );
					}
				}
				...
				do_action( "avadaredux/extension/customizer/control/includes" );

				//if ($this->parent->args['dev_mode']) {
				//    $section = new AvadaRedux_Customizer_rAds( $wp_customize, 'avadaredux_rAds', array(
				//        'priority'    => 0,
				//    ) );
				...
				//    ) );
				//
				//    //$wp_customize->add_control( new AvadaRedux_Customizer_Control_rAds( $wp_customize, 'avadareduxAdsDisplay', array(
				//    //    'section'        => 'avadaredux_rAds',
				//    //    'settings'       => 'avadaredux_rAds_field',
				//    //    'type'           => 'avadaredux-rAds',
				//    //) ) );
				//
				//
				//
				...
				if ( $this->parent->args['dev_mode'] ) {
					...
					//    'priority'    => 0,
					//);
					////$section = new AvadaRedux_Customizer_Section( $wp_customize, 'avadaredux_rAds', $args );
					////$wp_customize->add_section( $section, $args );
					//$this->add_section( 'avadaredux_rAds', array(
					//    'title'       => '',
					//    'priority'    => 1,
					//    'description' => '',
					...
					//    $wp_customize,
					//    'avadaredux_rAds_display',
					//    array(
					//        'section'    => 'avadaredux_rAds',
					//        'settings'   => 'avadaredux_rAds_display',
					//    )
					//));
					////$wp_customize->add_control( new AvadaRedux_Customizer_Control_rAds( $wp_customize, 'avadareduxAdsDisplay', array(
					////    'section'        => 'avadaredux_rAds',
					...
					//$section = new AvadaRedux_Customizer_section_rAds( $wp_customize, 'avadaredux_rAds', array(
					//    'priority'    => -999,
					//) );
					//$wp_customize->add_section( $section, array(
					//    'priority'    => -999,
					...
					//    'avadaredux_rAds_empty'
					//);
					//$wp_customize->add_control(
					//    new AvadaRedux_Customizer_Control_rAds(
					//        $wp_customize,
					//        'avadaredux_rAds_empty',
					//        array(
					//            'section'    => 'avadaredux_rAds',
					//            'settings'   => 'avadaredux_rAds_empty'
					//        )
					//    )
					//);
				}
				...
				foreach ( $this->parent->sections as $key => $section ) {
					...
					foreach ( $section['fields'] as $skey => $option ) {
						...
						add_action( 'avadaredux/advanced_customizer/control/render/' . $this->parent->args['opt_name'] . '-' . $option['id'], array(
							$this,
							'render'
						) );
						...
						$class_name = 'AvadaRedux_Customizer_Control_' . $option['type'];

						do_action( 'avadaredux/extension/customizer/control_init', $option );
						...
						$wp_customize->add_control( new $class_name( $wp_customize, $option['id'], array(
							...
							'section'        => $section['id'],
							'settings'       => $option['id'],
							'type'           => 'avadaredux-' . $option['type'],
							'field'          => $option,
							'AvadaReduxFramework' => $this->parent,
						...
						add_action( 'avadaredux/advanced_customizer/render/' . $option['id'], array(
							$this,
							'field_render'
						), $option['priority'] );
			...
			public function add_section( $id, $args = array(), $wp_customize ) {
				...
				if ( is_a( $id, 'WP_Customize_Section' ) ) {
					...
					$section_class = apply_filters( 'avadaredux/customizer/section/class_name', "AvadaRedux_Customizer_Section" );
			...
			public function add_panel( $id, $args = array(), $wp_customize ) {
				if ( is_a( $id, 'WP_Customize_Panel' ) ) {
					...
					$panel_class = apply_filters( 'avadaredux/customizer/panel/class_name', "AvadaRedux_Customizer_Panel" );
			...
			public function customizer_save_after( $wp_customize ) {
				...
				if ( $changed ) {
					...
					if ( $compiler ) {
						...
						do_action( "avadaredux/options/{$this->parent->args['opt_name']}/compiler", $this->parent->options, $this->parent->compilerCSS );
						do_action( "avadaredux/options/{$this->args['opt_name']}/compiler/advanced", $this->parent );
					}
				}
			...
			public function _enqueue_previewer() {
				wp_enqueue_script(
					'avadaredux-extension-previewer-js',
					$this->_extension_url . 'assets/js/preview.js'
				);

				$localize = array(
					'save_pending'   => __( 'You have changes that are not saved. Would you like to save them now?', 'avadaredux-framework' ),
					'reset_confirm'  => __( 'Are you sure? Resetting will lose all custom values.', 'avadaredux-framework' ),
					'preset_confirm' => __( 'Your current options will be replaced with the values of this preset. Would you like to proceed?', 'avadaredux-framework' ),
			...
			public function _enqueue() {
				...
				//wp_enqueue_script('avadaredux-extension-customizer-js', $this->_extension_url . 'assets/js/customizer.js');
				// Get styles
				//wp_enqueue_style('avadaredux-extension-customizer-css', $this->_extension_url . 'assets/css/customizer.css');
				...
				do_action( 'avadaredux-enqueue-' . $this->args['opt_name'] );
						...
						foreach ( $section['fields'] as $field ) {
							if ( isset( $field['type'] ) ) {
								...
								if ( ! class_exists( $field_class ) ) {
									$class_file = apply_filters( 'avadaredux-typeclass-load', $this->path . 'inc/fields/' . $field['type'] . '/field_' . $field['type'] . '.php', $field_class );
									if ( $class_file ) {
		...
		function avadaredux_customizer_custom_validation( $field ) {
			return $field;
		}
	} // if
