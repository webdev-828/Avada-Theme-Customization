(function($){
    ...
    avadaredux.field_objects.color_rgba          = avadaredux.field_objects.color_rgba || {};
    avadaredux.field_objects.color_rgba.fieldID  = '';

    avadaredux.field_objects.color_rgba.hexToRGBA = function( hex, alpha ) {
        var result;
    ...
    avadaredux.field_objects.color_rgba.init = function( selector ) {
        if ( !selector ) {
            selector = $( document ).find( ".avadaredux-group-tab:visible" ).find( '.avadaredux-container-color_rgba:visible' );
        }
        ...
        $( selector ).each(
            function() {
                ...
                avadaredux.field_objects.color_rgba.modInit(el);
                avadaredux.field_objects.color_rgba.initColorPicker(el);
            });
    };

    avadaredux.field_objects.color_rgba.modInit = function(el) {

        avadaredux.field_objects.color_rgba.fieldID    = el.find('.avadaredux-color_rgba-container').data('id');

    };

    // Initialize colour picker
    avadaredux.field_objects.color_rgba.initColorPicker = function(el){

        // Get field ID
        var field_id    = avadaredux.field_objects.color_rgba.fieldID;

        // Get the color scheme container
        var colorpickerInput = el.find('.avadaredux-color-rgba');
        ...
        var container   = el.find('.avadaredux-color-rgba-container');
        ...
        colorpickerInput.spectrum({
            ...
            className:              'avadaredux-color-rgba',
            ...
            localStorageKey:        'avadaredux.color-rgba.' + field_id,
            palette:                palette,
            inputText:              inputText,
            ...
            change: function(color) {
                ...
                if (colorVal != 'transparent') {
                    rgbaVal     = avadaredux.field_objects.color_rgba.hexToRGBA(colorVal, alphaVal);
                } else {
                    rgbaVal     = 'transparent';
                }
                ...
                avadaredux_change(el.find('.avadaredux-color-rgba-container'));
            }
        });
    };
})(jQuery);
