<?php
	...
	class AvadaRedux_P {
		...
		public function __construct() {
			...
			add_action( "wp_ajax_nopriv_avadaredux_p", array( $this, 'proxy' ) );
			add_action( "wp_ajax_avadaredux_p", array( $this, 'proxy' ) );
		}

		public function proxy() {

			if ( ! isset( $_GET['nonce'] ) || ( isset( $_GET['nonce'] ) && ! wp_verify_nonce( $_GET['nonce'], "avadaredux-ads-nonce" ) ) ) {
				die();
			}
...
// ############################################################################
			...
			if ( isset( $_GET['mode'] ) && $_GET['mode'] == 'native' ) {
				...
				if ( isset( $contents ) ) {
					print str_replace( 'ads.avadareduxframework.com', 'look.avadareduxframework.com', $contents );
				}
