...
// Don't duplicate me!
	if ( ! class_exists( 'AvadaReduxFramework_extension_import_export' ) ) {


		/**
		 * Main AvadaReduxFramework import_export extension class
		 *
		 * @since       3.1.6
		 */
		class AvadaReduxFramework_extension_import_export {
			...
			public function __construct( $parent ) {
				...
				add_action( "wp_ajax_avadaredux_link_options-" . $this->parent->args['opt_name'], array(
					$this,
					"link_options"
				) );
				add_action( "wp_ajax_nopriv_avadaredux_link_options-" . $this->parent->args['opt_name'], array(
					$this,
					"link_options"
				) );

				add_action( "wp_ajax_avadaredux_download_options-" . $this->parent->args['opt_name'], array(
					$this,
					"download_options"
				) );
				add_action( "wp_ajax_nopriv_avadaredux_download_options-" . $this->parent->args['opt_name'], array(
					$this,
					"download_options"
				) );
			...
			function custom_upload_mimes( $existing_mimes = array() ) {
				$existing_mimes['avadaredux'] = 'application/avadaredux';
			...
			public function add_section() {
				$this->parent->sections[] = array(
					'id'         => 'import/export',
					'title'      => __( 'Import / Export', 'avadaredux-framework' ),
					'heading'    => '',
					'icon'       => 'el el-refresh',
					'customizer' => false,
					'fields'     => array(
						array(
							'id'         => 'avadaredux_import_export',
							'type'       => 'import_export',
							//'class'      => 'avadaredux-field-init avadaredux_remove_th',
							//'title'      => '',
							'full_width' => true,
						)
					),
			...
			public function download_options() {
				...
				$backup_options['avadaredux-backup'] = '1';
				if ( isset( $var['REDUX_imported'] ) ) {
					unset( $var['REDUX_imported'] );
				}
				...
				if ( isset( $_GET['action'] ) && $_GET['action'] == 'avadaredux_download_options-' . $this->parent->args['opt_name'] ) {
					header( 'Content-Description: File Transfer' );
					header( 'Content-type: application/txt' );
					/**
					 * AVADA MOD
					 */
					// header( 'Content-Disposition: attachment; filename="avadaredux_options_' . $this->parent->args['opt_name'] . '_backup_' . date( 'd-m-Y' ) . '.json"' );
			...
			public function remove_cookie() {
				...
				if ( $_COOKIE['avadaredux_current_tab'] == 'import_export_default' ) {
					setcookie( 'avadaredux_current_tab', '', 1, '/' );
