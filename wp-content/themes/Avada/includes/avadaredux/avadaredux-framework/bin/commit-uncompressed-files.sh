if [[ "$TRAVIS_PULL_REQUEST" == "false" && "$TRAVIS_JOB_NUMBER" == *.1 ]]; then
	...
	v1=`find AvadaReduxCore -type f | sort -u | xargs cat | md5sum`
	echo "$v1"
	grunt compileCSS
	grunt compileJS
	v2=`find AvadaReduxCore -type f | sort -u | xargs cat | md5sum`
	echo "$v2"
	if [ "$v1" == "$v2" ]; then
		...
		#git remote set-url origin "https://$GH_TOKEN@github.com/AvadaReduxFramework/AvadaReduxFramework.git"
		#git status
