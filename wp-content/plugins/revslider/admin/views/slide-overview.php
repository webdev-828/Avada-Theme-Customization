...
if($slider->isSlidesFromPosts() == false){
	...
	$linkNewPost = RevSliderFunctions::getHtmlLink($urlNewPost, '<i class="revicon-pencil-1"></i>'.__('New Post','revslider'),'button_new_post','button-primary revblue',true);
	...
	<div class="wrap settings_wrap">
		<div class="title_line">
			...
			<div class="view_title"><?php _e('Edit Posts','revslider'); ?>: <?php echo $slider->getTitle(); ?></div>
		</div>
		<div class="vert_sap"></div>
		
		<?php _e("This is a list of posts that are taken from multiple sources.",'revslider'); ?> &nbsp;
		<?php if($showSortBy == true){ ?>
			<?php _e("Sort by",'revslider'); ?>: <?php echo $selectSortBy; ?> &nbsp; <span class="hor_sap"></span>
		<?php } ?>
		<?php echo $linkNewPost; ?>
		<span id="slides_top_loader" class="slides_posts_loader" style="display:none;"><?php _e("Updating Sorting...",'revslider'); ?></span>
		<div class="vert_sap"></div>
		<div class="sliders_list_container">
			<div class="postbox box-slideslist">
				<h3>
					<span class='slideslist-title'><?php _e('Post List','revslider'); ?></span>
					<span id="saving_indicator" class='slideslist-loading'><?php _e("Saving Order",'revslider'); ?>...</span>
				</h3>
				<div class="inside">
					<?php if(empty($arrSlides)){?>
						<?php _e('No Posts Found','revslider'); ?>
					<?php } ?>
							...
							<li id="slidelist_item_<?php echo $postID; ?>" class="ui-state-default">
								<span class="slide-col col-order">
									...
									<?php if($state == "published"){ ?>
										<div class="icon_state state_published" data-slideid="<?php echo $postID; ?>" title="<?php _e("Unpublish Post",'revslider'); ?>"></div>
									<?php }else{ ?>
										<div class="icon_state state_unpublished" data-slideid="<?php echo $postID; ?>" title="<?php _e("Publish Post",'revslider'); ?>"></div>
									<?php } ?>
								</span>
								<span class="slide-col col-name">
									<div class="slide-title-in-list"><?php echo $linkEdit; ?></div>
									<a class='button-primary revgreen' href='<?php echo $urlEditSlide; ?>'><i class="revicon-pencil-1"></i><?php _e("Edit Post",'revslider'); ?></a>
								</span>
								<span class="slide-col col-image">
									<?php if(!empty($urlImageForView)){ ?>
										<div id="slide_image_<?php echo $postID; ?>" class="slide_image" title="<?php _e('Click to change the slide image. Note: The post featured image will be changed.', 'revslider'); ?>" alt="<?php echo $imageAlt; ?>" style="background-image:url('<?php echo $urlImageForView; ?>');<?php echo $bgStyle; ?>"></div>
									<?php }else{ ?>
										<div id="slide_image_<?php echo $postID; ?>" class="slide_image" title="<?php _e('Click to change the slide image. Note: The post featured image will be changed.', 'revslider'); ?>" alt=""><?php _e('no image', 'revslider'); ?></div>
									<?php } ?>
								</span>
								<span class="slide-col col-operations-posts">
									<a id="button_delete_slide" class='button-primary revred button_delete_slide' data-slideid="<?php echo $postID; ?>" href='javascript:void(0)'><i class="revicon-trash"></i><?php _e("Delete",'revslider'); ?></a>
								</span>
		...
		<div class="list_slides_bottom">
			<?php echo $linkNewPost; ?>
			<a class="button-primary revyellow" href='<?php echo self::getViewUrl(RevSliderAdmin::VIEW_SLIDERS); ?>' ><i class="revicon-cancel"></i><?php _e("Close",'revslider'); ?></a>
			<a href="<?php echo $linksSliderSettings; ?>" class="button-primary revgreen"><i class="revicon-cog"></i><?php _e("Slider Settings",'revslider'); ?></a>
		</div>
	</div>
	<script type="text/javascript">
		var g_messageDeleteSlide = "<?php _e("Warning! Removing this entry will cause the original wordpress post to be deleted.",'revslider'); ?>";
		var g_messageChangeImage = "<?php _e("Select Slide Image",'revslider'); ?>";
